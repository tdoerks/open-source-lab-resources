#!/usr/bin/env python3
"""
Fetch Salmonella enterica SRA Accessions from NCBI
50 random samples per month from January 2020 to March 2026

Focus: Prophage-virulence-AMR dynamics by serotype
Organism: Salmonella enterica (high prophage burden, serotype diversity)
"""
import requests
import random
import time
...
def fetch_sra_accessions(year, month, max_results=500):
    """Fetch SRA accessions for Salmonella enterica from a specific month"""
    ...
    params = {
        'db': 'sra',
        'term': f'Salmonella enterica[Organism] AND {date_str}[Release Date] AND illumina[Platform] AND GENOMIC[Source] AND WGS[Strategy]',
        'retmax': max_results,
        'retmode': 'json'
    }
...
def main():
    print("="*70)
    print("Salmonella Prophage Dynamics - Multi-Serotype Temporal Analysis")
    print("="*70)
    print("Organism: Salmonella enterica")
    print("Period: January 2020 - March 2026")
    print("Target: 50 random samples per month")
    print("Focus: Prophage-virulence-AMR dynamics by serotype")
    print("="*70)
    print()
    ...
    # Save to file
    output_file = "data/sra_accessions_salmonella_monthly_50_2020-2026.txt"
    ...
    print()
    print("Expected Serotypes (based on typical distributions):")
    print("  - Typhimurium: ~20-25%")
    print("  - Enteritidis: ~15-20%")
    print("  - Newport: ~10-15%")
    print("  - Javiana, I 4,[5],12:i:-, Heidelberg: ~5-10% each")
    print("  - Others: ~20-30%")
    print()
    print("Next steps:")
    print(f"  1. Review: head {output_file}")
    print(f"  2. Generate samplesheet: python3 scripts/create_samplesheet.py")
    print(f"  3. Run COMPASS: sbatch run_salmonella_temporal_phage.sh")
    print("="*70)
